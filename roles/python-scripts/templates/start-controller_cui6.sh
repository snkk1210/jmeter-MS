...
# スレッド数
THREAD_NUM=`grep "ThreadGroup.num_threads" ${FILE_JMX} | sed -e "s/^.*<stringProp.*>\(.*\)<\/stringProp>.*$/\1/" | head -n 1`
# ランプアップ数
RAMPUP_NUM=`grep "ThreadGroup.ramp_time" ${FILE_JMX} | sed -e "s/^.*<stringProp.*>\(.*\)<\/stringProp>.*$/\1/" | head -n 1`
# ループ回数
LOOP_NUM=`grep "LoopController.loops" ${FILE_JMX} | sed -e "s/^.*<stringProp.*>\(.*\)<\/stringProp>.*$/\1/" | head -n 1`
# 無限ループ
#LOOP_NUM=`grep "LoopController.loops" ${FILE_JMX} | sed -e "s/^.*<intProp.*>\(.*\)<\/intProp>.*$/\1/" | head -n 1`
# ノード数
